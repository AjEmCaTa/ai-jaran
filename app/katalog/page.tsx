...
export default function KatalogPage() {
  ...
  // Učitavanje rezervacija / zauzetih termina sa baze
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.success && Array.isArray(data.data)) {
          // Mapiramo podatke iz baze u format koji aplikacija koristi
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        console.error("SERVER VRATIO GREŠKU:", data);
        alert("Greška sa servera: " + (data.error || "Nepoznata greška"));
        return;
      }
      ...
      // Osvježi listu rezervacija iz baze
      fetchMyReservations();

    } catch (error: any) {
      console.error("Greška pri slanju na server:", error);
      alert("Greška pri slanju na server: " + error.message);
    }
  };
  ...
  const cancelReservation = async (id: number) => {
    try {
      ...
      if (res.ok) {
        ...
        // Fallback ako ruta očekuje drugačiji format
        setMyReservations(myReservations.filter(r => r.id !== id));
      }
    } catch (err) {
